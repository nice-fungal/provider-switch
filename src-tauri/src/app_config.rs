...
pub struct MultiAppConfig {
    ...
    #[serde(default)]
    pub mcp: McpRoot,
    /// 旧版 Prompts 配置占位（功能已移除）。
    /// 仅用于吞掉旧配置文件中的 `prompts` 键——`apps` 是 flatten map，若没有
    /// 该命名字段，旧配置里的 `prompts` 会被误吸收为 ProviderManager 导致加载失败。
    /// 永不写回磁盘。
...
mod tests {
    ...
    #[test]
    fn app_type_supports_kilo() {
        assert_eq!(AppType::Kilo.as_str(), "kilo");
        assert_eq!("kilo".parse::<AppType>().unwrap(), AppType::Kilo);
    }
